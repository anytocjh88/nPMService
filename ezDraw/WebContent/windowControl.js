...
function person_down(param){
	...
	for(var i = 1; i <= 6; i++){
		document.getElementById('person' + i).style.background = '#2b2b2b';
	}
	//선택된 이미지의 배경색을 변경한다
	document.getElementById(param).style.background = '#828282';
}
...
function font_down(param){
	...
	//활성 -> 비활성
	if(bgcolor == 'rgb(130, 130, 130)'){
		document.getElementById(param).style.background = '#2b2b2b';
		//글꼴의 스타일을 변수에 저장
		if(param == 'fontBold'){
			isFontBold = false;
		}
